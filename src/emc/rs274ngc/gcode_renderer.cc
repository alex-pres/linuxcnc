...
void GCodeRenderer::set_xy_rotation(double degrees) {
    ...
    // double back = -rad;
    unrot_cos_ =  rotation_cos_; // cos(back);
...
bool PreviewData::reserve(size_t extra) {
    ...
    for(int i = 0; i < nplanes; i++) {
        grown[i] = (float*)realloc(pos[i], want * P3_COUNT * sizeof(*pos[i]));
        if(!grown[i]) {
    ...
    uint32_t *grown_attrs =
        (uint32_t*)realloc(attrs, want * ATTRS_PER_VERTEX * sizeof(*attrs));
    if(!grown_attrs) {
    ...
    if(want_axis_positions && n_axes) {
        grown_axes = (float*)realloc(axis_pos, want * (size_t)n_axes * sizeof(*axis_pos));
        if(!grown_axes) {
            for(int i = 0; i < nplanes; i++) pos[i] = grown[i];
            attrs = grown_attrs;
            return false;
...
// An offset arriving at a row another already claims governs no row, so it
// overwrites it: that is what keeps the rows increasing.
void PreviewData::set_tool_offset(const Point9 &offsets) {
    if(!tool_offsets.empty() && tool_offsets.back().index == n)
        tool_offsets.back().offsets = offsets;
    else
        tool_offsets.push_back(ToolOffsetRecord{n, offsets});
}

// One left past the last vertex governs no row either, so it is dropped.
void PreviewData::drop_trailing_tool_offset() {
    if(!tool_offsets.empty() && tool_offsets.back().index >= n)
        tool_offsets.pop_back();
}
...
void PreviewData::shrink() {
    ...
    size_t want = n ? n : 1;
    for(int i = 0; i < nplanes; i++) {
        float *fit = (float*)realloc(pos[i], want * P3_COUNT * sizeof(*pos[i]));
        if(fit) pos[i] = fit;           // a refused shrink keeps the slack
    }
    uint32_t *fit = (uint32_t*)realloc(attrs, want * ATTRS_PER_VERTEX * sizeof(*attrs));
    if(fit) attrs = fit;
    if(axis_pos) {
        float *fit_axes =
            (float*)realloc(axis_pos, want * (size_t)n_axes * sizeof(*axis_pos));
    ...
    py::class_<PreviewData>(m, "PreviewGeometry",
        ...
        .def("positions", [](py::object self, int plane) {
                ...
                return array_view(std::move(self), d.pos[plane],
                                  (Py_ssize_t)d.n * P3_COUNT, sizeof(*d.pos[plane]), "f");
            }, py::arg("plane") = 0,
        ...
        .def("tool_offsets", [](const PreviewData &d) {
                ...
                for(const ToolOffsetRecord &r : d.tool_offsets)
                    out.append(py::make_tuple(r.index, nine(r.offsets)));
                return out;
            }, "(row it takes effect at, nine offsets) per tool offset; "
               "rows increasing, each holding until the next")
        .def("axis_positions", [](py::object self) {
                ...
                Py_ssize_t f = sizeof(*d.axis_pos);
...
// `RendererCanon.transform`: where the parse in flight is putting points
// right now. The offsets it applies move as the parse walks, so a canon
// asking at a `(WORKPIECE,...)` gets the frame that comment was written in -
// which is the whole point of asking the renderer instead of reproducing the
// chain in Python.
//
// Sized for the handful of points a comment declares, not for a program: each
// number crosses the boundary one at a time.
static py::tuple point_rows(const std::vector<double> &xyz) {
    py::tuple out(xyz.size() / P3_COUNT);
    for(size_t i = 0; i < xyz.size() / P3_COUNT; i++)
        out[i] = py::make_tuple(xyz[i * P3_COUNT], xyz[i * P3_COUNT + 1],
                                xyz[i * P3_COUNT + 2]);
    return out;
}
...
static py::tuple renderer_transform(py::handle self, py::handle points) {
    ...
    // The parse transforms *its* canon's points. A canon left over from an
    // earlier parse would otherwise read this one's offsets and be believed.
    ...
    if(!parse_state.canon->transform_points(in.data(), in.size(),
                                            machine.data(), display.data()))
        throw py::value_error("transform: this parse does not transform");
...
void renderer_canon_register(py::module_ &m) {
    ...
    ns["__doc__"] =
            ...
            "During a parse, `self.transform(points)` returns those points\n"
            "in machine and in display coordinates - where a move endpoint on\n"
            "the same line would land - so geometry a comment declares can be\n"
            "placed without reproducing the offset chain. Points are 3 or 9\n"
            "numbers each; the answer is the offsets as they stand when it is\n"
            "asked, and it is a few points' worth of work, not a program's.\n\n"
    ...
    cls.attr("transform") = py::cpp_function(&renderer_transform,
            py::is_method(cls), py::arg("points"),
...
std::unique_ptr<GCodeRenderer> GCodeRenderer::make(PyObject *canon_ptr) {
    ...
    for(int i = 0; i < EXTENT_KINDS; i++)
        for(int j = 0; j < P3_COUNT; j++) {
            r->data_->extents[i][BOX_MIN][j] = DBL_MAX;
            r->data_->extents[i][BOX_MAX][j] = -DBL_MAX;
        }
    for(int j = 0; j < P3_COUNT; j++) {
        r->data_->drawn[BOX_MIN][j] = DBL_MAX;
        r->data_->drawn[BOX_MAX][j] = -DBL_MAX;
    }
...
// carrying an A/B/C value is turned by its own angle exactly as its move
// would be, and a GEOMETRY naming U/V/W reads them off the point rather than
// losing them.
bool GCodeRenderer::transform_points(const Point9 *in, size_t n,
                                     double *machine, double *display) const {
    for(size_t i = 0; i < n; i++) {
        Point9 at;
        transform(in[i], at);
        machine[i * P3_COUNT + P3_X] = at[P9_X];
        machine[i * P3_COUNT + P3_Y] = at[P9_Y];
        machine[i * P3_COUNT + P3_Z] = at[P9_Z];
        Point3 drawn;
        plane_point(data_->ops[0], at, drawn);
