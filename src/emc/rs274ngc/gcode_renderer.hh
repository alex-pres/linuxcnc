...
// A tool offset. It draws nothing, so the row it takes effect at addresses
// it and it holds until the next record's row. A line number could not: one
// line can set it many times, or straddle its own change.
struct ToolOffsetRecord {
    size_t index;                       // first row written under it
...
public:
    ...
    bool transform_points(const Point9 *in, size_t n,
                          double *machine, double *display) const override;
